...
public final class BoardImageCollection {
    public final Map<Byte, Image> dirImages;

    /**
     * Constructs an image collection with the images that are found in the directory.
     * The images are associated with the 2-digit number found at the beginning of their name.
     * If files fail to be read, they are simply ignored.
     */

    public BoardImageCollection() {
        Map<Byte, Image> images = new HashMap<>();
        File dir = new File(getClass().getResource("/boardImages").getFile());
